...
export default function Competitive() {
    ...
    useEffect(() => {
        axios.get(`${API}/brands`).then(r => setAllBrands(r.data.brands)).catch(() => { });
    }, []);

    useEffect(() => {
        if (selected.length === 0) return;
        setLoading(true);
        axios.get(`${API}/competitive`, { params: { brands: selected.join(","), hours: 48 } })
            .then(r => setData(r.data.brands))
            .catch(() => { })
    ...
    const chartData = data ? Object.entries(data).map(([brand, m]) => ({
        brand,
