...
export default function Navbar() {
  ...
  useEffect(() => {
    // Use cached getAlerts — no extra network call if Dashboard already fetched
    const poll = () => getAlerts().then(a => setAlertCount(a.length)).catch(() => {});
    poll();
    const id = setInterval(() => getAlerts({ force: true }).then(a => setAlertCount(a.length)).catch(() => {}), 60_000);
