...
export default function TextAnalysis() {
  ...
  async function analyze() {
    ...
    setError("");
    try {
      const data = await predict(text, platform);
      setResult(data);
    } catch (e) {
        ...
        <div style={card()}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 16 }}>
            <button onClick={analyze} disabled={loading || !text.trim()} style={{
              ...
              cursor: loading ? "not-allowed" : "pointer", opacity: loading || !text.trim() ? 0.5 : 1,
            }}>
              {loading ? "Analysing..." : "▶  Analyse"}
            </button>
            <span style={mono(10, C.t4)}>or Ctrl+Enter</span>
          </div>

          {error && (
            <div style={{ marginTop: 14, padding: "10px 16px", borderRadius: 8, background: "rgba(255,77,112,0.08)", border: `1px solid rgba(255,77,112,0.3)`, fontFamily: C.sans, fontSize: 12, color: C.neg }}>
              {error}
            </div>
          )}
        </div>
            ...
            <>
              ...
              {result.brands?.length === 0 && (
                <div style={{ padding: "14px 20px", borderRadius: 12, background: "rgba(245,200,66,0.06)", border: `1px solid rgba(245,200,66,0.25)`, display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontFamily: C.sans, fontSize: 13, color: C.neu }}>No brand detected in this post</span>
                </div>
              )}
